...
class CastepConvergeWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.outline(
            ...
            while_(cls.should_conv_supercell) (cls.run_supercell_conv, cls.analyse_supercell_conv),
            cls.results)
    ...
    def setup(self):
        ...
        if self.inputs.converge_supercell:
            self.ctx.initial_supercell_length = self.inputs.initial_supercell_length.value
    ...
    def run_pwcutoff_conv(self):
        ...
        for pwcutoff in range (self.ctx.initial_pwcutoff, self.ctx.final_pwcutoff + 1, self.inputs.pwcutoff_step.value):
    ...
    def analyse_pwcutoff_conv(self):
        ...
        for pwcutoff in range(self.ctx.initial_pwcutoff, self.ctx.final_pwcutoff + 1, self.inputs.pwcutoff_step.value):
            ...
            if energy_diff_per_atom < self.inputs.energy_error:
                self.report(f'Plane-wave energy cutoff converged at {pwcutoff - self.inputs.pwcutoff_step.value} eV')
                self.ctx.converged_pwcutoff = orm.Int(pwcutoff - self.inputs.pwcutoff_step.value)
                self.ctx.pwcutoff_converged = True
                return
    ...
    def analyse_kspacing_conv(self):
        ...
        self.report('K-point spacing not converged. Decreasing lower limit by 0.02 A-1.')
    ...
    def run_supercell_conv(self):
        ...
        inputs = self.ctx.inputs
        parameters = deepcopy(self.ctx.parameters)
        parameters.update({'task':'phonon', 'phonon_fine_method':'supercell', 'cut_off_energy':self.inputs.initial_pwcutoff.value})
        inputs.calc.parameters = parameters
        ...
        supercell_lengths = np.arange(self.ctx.initial_supercell_length, self.ctx.final_supercell_length + 0.1, self.inputs.supercell_step.value)
    ...
    def analyse_supercell_conv(self):
        ...
        for i in range(len(supercell_lengths)):
            ...
            if key in self.ctx:
                ...
                if len(keys) == 1:
                        continue
                last_phonon_file = self.ctx[key].called[-1].outputs.retrieved.get_object_content('aiida.phonon')
                with TemporaryDirectory() as temp:
                    ...
                    with open(f'{temp}/second_last.phonon','x') as file:
                        file.write(last_phonon_file)
                    second_last_phonon_data = PhononParser(open(f'{temp}/second_last.phonon'))
                    second_last_freqs = np.array(second_last_phonon_data.frequencies)
                percentage_errors = np.absolute((second_last_freqs - last_freqs)) / last_freqs
                mean_percentage_error = np.mean(percentage_errors)
                if mean_percentage_error < (self.inputs.frequency_error / 100):
                    self.report(f'Supercell converged at {supercell_lengths[i - 1]} Angstroms')
                    self.ctx.supercell_converged = True
                    self.ctx.converged_supercell = orm.ArrayData()
                    self.ctx.converged_supercell.set_array('matrix', np.array(self.ctx[keys[-2]].inputs.calc.parameters['phonon_supercell_matrix']))
                    return
        self.ctx.initial_supercell_length = self.ctx.final_supercell
        self.ctx.final_supercell_length += 5
        self.report('Supercell not converged. Increasing upper length limit by 5 Angstroms')
