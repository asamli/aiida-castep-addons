...
@calcfunction
def phonon_analysis(prefix, ir_folder, kpoints, raman_folder, structure, uuid, label, description):
    ...
    return {'band_data':band_data, 'bands':band_plot, 'ir_data':ir_data, 'ir_spectrum':ir_spectrum, 'raman_data':raman_data, 'raman_spectrum':raman_spectrum}
...
class CastepPhononWorkChain(WorkChain):
    ...
    def run_phonon(self):
        ...
        if self.inputs.use_supercell:
            ...
            phonon_parameters.update({'phonon_fine_method':'interpolate', 'fix_occupancy':True})
    ...
    def analyse_phonons(self):
        ...
        outputs = phonon_analysis(orm.Str(self.ctx.prefix), self.ctx.phonon.called[-1].outputs.retrieved, self.ctx.band_kpoints, 
                                  self.ctx.raman.called[-1].outputs.retrieved, self.ctx.inputs.calc.structure, orm.Str(self.uuid), 
