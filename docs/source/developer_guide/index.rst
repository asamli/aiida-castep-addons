...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-castep-addons`` repository (preferably using ``aiida-castep-addons`` as the project name)

The documentation is now available at `aiida-castep-addons.readthedocs.io <http://aiida-castep-addons.readthedocs.io/>`_.
...
.. note::
   ...
   When updating the plugin package to a new version, remember to update the version number both in ``setup.json`` and ``aiida_castep_addons/__init__.py``.
